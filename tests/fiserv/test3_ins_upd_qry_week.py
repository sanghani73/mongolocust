...
class MongoSampleUser(MongoUser):
    ...
    def find_authorisation_7_days(self):
        ...
        filter={
            ...
            'posting_date': {
                '$gte': datetime(2020, month, day, 0, 0, 0, tzinfo=timezone.utc), 
                '$lte': datetime(2020, month, end_day, 0, 0, 0, tzinfo=timezone.utc)
            }
        }
        sort=list({
            'posting_date': -1
    ...
    def do_find_authorisation(self):
        self._process('find-authorisation-7_days', self.find_authorisation_7_days)
